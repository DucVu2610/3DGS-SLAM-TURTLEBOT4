...
class MAGiCSLAM(object):
    ...
    def run(self) -> None:
        ...
        for agent_id in self.agent_ids:
            agent_submaps = []
            output_path = Path(self.config["data"]["output_path"]) / f"agent_{agent_id}"
            for submap_path in sorted((output_path / "submaps").glob("*")):
                agent_submap = torch.load(submap_path, map_location="cpu", weights_only=False)
                agent_submaps.append(agent_submap)
            self.agents_messages[agent_id] = "pgo"
        ...
        if self.config["dataset_name"] == "aria":  # we only have hold-out views in Aria
            ...
            novel_psnr_mean = np.mean(np.array(novel_psnr))
            novel_ssim_mean = np.mean(np.array(novel_lpips))
            novel_lpips_mean = np.mean(np.array(novel_ssim))
            novel_depth_mean = np.mean(np.array(novel_dl1))
            print("Eval novel views")
            print(f"{novel_psnr_mean:.3f}, {novel_ssim_mean:.3f}, {novel_lpips_mean:.3f}, {novel_depth_mean:.3f} \n")
            save_dict_to_json({"psnr": novel_psnr_mean,
                               "lpips": novel_lpips_mean,
                               "ssim": novel_ssim_mean,
                               "depth_l1": novel_depth_mean},
